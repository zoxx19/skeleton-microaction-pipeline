...
4. **`normalize_updated.py`** — a second, improved version of step 3 that additionally
   ...
   transform. **Only run for one subject (`VP_04`) so far** — see
...
- **`normalize_updated.py` (v2 normalization) is only validated on one subject
  (`VP_04`)**, not the full 21-subject cohort. The v1 `normalize.py` output is complete
  for all subjects; v2 (which adds orientation alignment on top of v1) has not yet been
  run/verified at full scale.
- **Segmentation threshold choice.** Movement segmentation (`segment.py`) uses a
  per-subject 85th-percentile activity threshold, computed across all task phases and
  then applied per-phase. Simpler alternatives (a fixed per-joint threshold; a per-phase
  percentile) were tried first and rejected — a fixed threshold misclassified nearly all
  or none of the frames as "moving" since subjects are rarely fully still, and a
  per-phase-only percentile wasn't comparable across different task conditions.
